...
module.exports = (client, message, member) => {
    ...
    const boosterEmbed = new MessageEmbed() 
        .setAuthor(`${member.user.tag} vient de booster le serveur !`, member.displayAvatarURL())
        .setDescription(`<a:boost:988386530986962954> **Merci beaucoup** à toi ${member.displayName}, grâce à toi le serveur est au niveau **${ct.premiumTier === "NONE" ? "0" : ct.premiumTier === "TIER_1" ? "1" : ct.premiumTier === "TIER_2" ? "2" : "3"}** avec **${ct.premiumSubscriptionCount > 1 ? ct.premiumSubscriptionCount + "** boosts" : ct.premiumSubscriptionCount + "** boost"} ! <a:tada:989221792818282567> \n\n Pour fêter ça, je t'offre **1000 points d'expérience** !`)
        .setFooter(`Merci beaucoup ${member.displayName} !`, member.displayAvatarURL())
